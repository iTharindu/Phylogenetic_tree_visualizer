...
def get_circular_tree_data(tree, order='level', dist=1, start_angle=0, end_angle=360, start_leaf='first'):
    ...
    def get_vertical_position(tree):
        ...
        if start_leaf == 'first':
            node_ycoord = dict((leaf, k)
                               for k, leaf in enumerate(tree.get_terminals()))
        elif start_leaf == 'last':
            node_ycoord = dict((leaf, k) for k, leaf in enumerate(
                reversed(tree.get_terminals())))
        else:
...
def create_circular_tree():
    ...
    for k in range(len((all_clades))):
        all_clades[k].id = k
    xnodes, ynodes,  xlines, ylines, xarc, yarc = get_circular_tree_data(
        tree, order=traverse_order, start_leaf='last')
    ...
    for clade in tree.find_clades(order=traverse_order):
        if clade.name and clade.confidence and clade.branch_length:
            ...
            tooltip.append(
                f"id: {clade.id}<br>name: {clade.name}<br>branch-length: {clade.branch_length}")
            color.append(-1)
        else:
            tooltip.append('')
            color.append(-1)
    ...
    pl_colorscale = [[0.0, 'rgb(10,10,150)'],  # color for leafs that haven't associated a confidence
                     [0.001, 'rgb(10,10,150)'],
                     # in fact the colorscale starts here
                     [0.001, 'rgb(214, 47, 38)'],
                     [0.1, 'rgb(214, 47, 38)'],
                     [0.2, 'rgb(244, 109, 67)'],
                     [0.3, 'rgb(252, 172, 96)'],
    ...
    fig = go.FigureWidget(
        data=[trace_radial_lines, trace_arcs, trace_nodes], layout=layout)

    py.sign_in('batsy', 'kjBbhbBDKt0M5xkXVaGB')
    py.plot(fig, filename='new_phyloxml')

    tls.get_embed('https://plot.ly/~Batsy/0')
